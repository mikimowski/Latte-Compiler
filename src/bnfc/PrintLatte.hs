...
instance Print (TopDef a) where
  prt i e = case e of
    ...
    TopFuncDef _ funcdef -> prPrec i 0 (concatD [prt 0 funcdef])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
instance Print (FuncDef a) where
  prt i e = case e of
    FuncDef _ type_ id args block -> prPrec i 0 (concatD [prt 0 type_, prt 0 id, doc (showString "("), prt 0 args, doc (showString ")"), prt 0 block])
...
instance Print (ClassMember a) where
  prt i e = case e of
    ClassField _ type_ id -> prPrec i 0 (concatD [prt 0 type_, prt 0 id, doc (showString ";")])
    ClassMethod _ funcdef -> prPrec i 0 (concatD [prt 0 funcdef])
  prtList _ [] = (concatD [])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
instance Print (ClassDef a) where
  prt i e = case e of
    ClassDef _ id classmembers -> prPrec i 0 (concatD [doc (showString "class"), prt 0 id, doc (showString "{"), prt 0 classmembers, doc (showString "}")])
    ClassExtDef _ id1 id2 classmembers -> prPrec i 0 (concatD [doc (showString "class"), prt 0 id1, doc (showString "extends"), prt 0 id2, doc (showString "{"), prt 0 classmembers, doc (showString "}")])
...
instance Print (Type a) where
  prt i e = case e of
    TClass _ id -> prPrec i 0 (concatD [prt 0 id])
    ...
    TArray _ type_ -> prPrec i 0 (concatD [prt 0 type_, doc (showString "[]")])

instance Print (Expr a) where
  prt i e = case e of
    ENewObject _ type_ -> prPrec i 6 (concatD [doc (showString "new"), prt 0 type_])
    ...
    EArrGet _ expr1 expr2 -> prPrec i 6 (concatD [prt 6 expr1, doc (showString "["), prt 0 expr2, doc (showString "]")])
    ECast _ id -> prPrec i 5 (concatD [doc (showString "cast"), doc (showString "<"), prt 0 id, doc (showString ","), doc (showString "null"), doc (showString ">")])
